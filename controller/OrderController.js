...
function updateOrderDetailsTable() {
    let tbody = $('#orderDetailsTable tbody');
    tbody.empty();
    order_details_db.forEach((detail) => {
        let order = order_db.find(order => order.orderId === detail.orderId);
        let orderDate = order ? order.orderDate : '';
        let customerId = order ? order.customerId : '';
        let itemCode = detail.itemId;
        let qty = detail.qty;
        let price =detail.unitPrice;
        let discountValue = parseFloat(discount.val()) || 0;
        let subTotal = qty * price * ((100 - discountValue) / 100);
